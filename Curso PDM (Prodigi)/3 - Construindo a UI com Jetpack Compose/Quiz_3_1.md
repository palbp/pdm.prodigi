...
3. Qual afirmação que melhor descreve como a UI reage às mudanças nos dados em Jetpack Compose?
    ...
    - Quando os dados observados mudam, a framework realiza automaticamente a recomposição dos composables em causa.
    - A recomposição só ocorre em reconfigurações (p.ex., rotação) ou se for chamado um método especial da Activity.
    - A recomposição depende apenas de alterações no layout; mudanças nos dados não a desencadeiam.
